...
export const SidebarSection = ({ searchQuery, setSearchQuery }: SidebarSectionProps): JSX.Element => {
  return (
    <header className="w-full h-12 bg-[#f9f5ff] dark:bg-gray-800 shadow-[0px_2px_6px_#b9b5c0a6] dark:shadow-[0px_2px_6px_rgba(0,0,0,0.3)] flex items-center justify-between px-[91px] transition-colors duration-300">
      <motion.h1 
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        className="font-semibold text-[#7336e6] dark:text-indigo-400 text-xl [font-family:'DM_Sans',Helvetica] tracking-[0] transition-colors duration-300"
      >
        Ticket List
      </motion.h1>
      ...
      <div className="flex items-center gap-2 ml-[170px]">
        <motion.div 
          ...
          className="flex w-[281px] items-center justify-between px-4 py-1.5 bg-white dark:bg-gray-700 rounded-[40px] border border-solid border-[#dad4ea] dark:border-gray-600 transition-colors duration-300"
        >
          <Input
            type="text"
